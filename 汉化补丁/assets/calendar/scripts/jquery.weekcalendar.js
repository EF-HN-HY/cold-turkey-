  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
        ...
        eventHeader: function(calEvent, calendar) {
          ...
          if (displayTitleWithTime) {
            ...
            return calendar.weekCalendar('formatTime', calEvent.start) +
                    ...
                    calendar.weekCalendar('formatTime', calEvent.end);
          }
        },
      ...
      /*
        ...
        */
      removeEvent: function(eventId, replaceIndex) {
          ...
          self.element.find('.wc-cal-event').each(function() {
            if ($(this).data('calEvent').id === eventId) {
                $(this).remove();
                /*return false; Removed for CT */
				
			/* Below was added for CT */
            } else if ($(this).data('calEvent').schedulerId > replaceIndex) {
				$(this).data('calEvent').schedulerId = $(this).data('calEvent').schedulerId - 1;
			}
			/* Above was added for CT */
      ...
      getTimeslotTimes: function(date) {
          ...
          for (var i = 0; i < options.timeslotsPerDay; i++) {
			...
			var endDateCheck = new Date(endMillis);

			if (endDateCheck.getDate() == startDate.getDate() || (endDateCheck.getHours() == 0 && endDateCheck.getMinutes() == 0)) {
				times[i] = {
      ...
      _renderCalendarHeader: function($calendarContainer) {
        ...
        for (var i = 1; i <= options.daysToShow; i++) {
          calendarHeaderHtml += '<td class=\"subtitle font-green-sharp wc-day-column-header wc-day-' + i + '\"' + colspan + '></td>';
        }
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderEvents: function(data, $weekDayColumns) {
        ...
        $.each(eventsToRender, function(i, calEvent) {
		  ...
		  /* CHANGES FOR Cold Turkey: code below is f**ked up (infinite loop)... CT will prevent multiday anyway... */
          ...
          if (start <= initialEnd) {
			...
			$weekDay = self._findWeekDayForEvent(calEvent, $weekDayColumns); /* Added here by Cold Turkey */
			self._renderEvent(calEvent, $weekDay); /* Added here by Cold Turkey */
			nbRenderedEvents += 1; /* Added here by Cold Turkey */
      ...
      _findWeekDayForEvent: function(calEvent, $weekDayColumns) {
          ...
          $weekDayColumns.each(function(index, curDay) {
            if ($(this).data('startDate').getTime() <= calEvent.start.getTime() &&
                  /*$(this).data('endDate').getTime() >= calEvent.end.getTime() && Cold Turkey Change*/
                  (!showAsSeparatedUser || $.inArray($(this).data('wcUserId'), user_ids) !== -1)
            ) {
                if ($weekDay) {
                /*$weekDay = $weekDay.add($(curDay)); Cold Turkey Change*/
                $weekDay = $(curDay); /*Cold Turkey Change*/
                }
                else {
                $weekDay = $(curDay);
                }
